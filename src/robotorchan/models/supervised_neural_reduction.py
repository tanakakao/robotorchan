...
class SupervisedAutoEncoderInputReducer(AutoEncoderInputReducer):
    ...
    The reducer minimizes a weighted sum of input reconstruction loss and an
    auxiliary outcome-prediction loss. The learned encoder is frozen after
    pretraining, so attaching the reducer to ``ReducedGP`` retains the existing
    frozen-representation lifecycle while allowing ``Y`` to shape the latent
    coordinates.

    Args:
        latent_dim: Number of latent input dimensions.
        supervised_weight: Weight applied to the standardized outcome MSE.
        standardize_y: Whether to standardize outcomes for the auxiliary loss.
        kwargs: Arguments forwarded to ``AutoEncoderInputReducer``.
    """
