"""Multi-fidelity GP wrappers with robotorchan model conventions."""
...
class MixedSingleTaskMultiFidelityGP(SingleTaskMultiFidelityGP):
    """Multi-fidelity GP for mixed continuous/categorical design spaces.

    The mixed covariance models only non-fidelity input columns. BoTorch's
    native iteration- and data-fidelity kernels are retained and multiplied
    with that mixed data covariance.

    ``linear_truncated=True`` is intentionally unsupported because BoTorch's
    linear-truncated fidelity kernel owns the complete input covariance and
    does not accept a separate covariance module for non-fidelity features.
    """
    ...
    def __init__(
        ...
        if linear_truncated:
            raise ValueError(
                ...
                "because BoTorch's linear-truncated fidelity kernel does not "
                "accept a separate non-fidelity covariance module."
            )

        input_dim = train_X.shape[-1]
        normalized_cat_dims = _normalize_cat_dims(
            cat_dims=cat_dims,
            input_dim=input_dim,
        )
        ...
        if iteration_fidelity is not None:
            fidelity_dims.extend(
                _normalize_dims(
                    [iteration_fidelity],
                    input_dim,
                    name="Iteration fidelity",
                )
            )
        if data_fidelities is not None:
            fidelity_dims.extend(
                _normalize_dims(
                    list(data_fidelities),
                    input_dim,
                    name="Data fidelity",
                )
            )
        ...
        if set(normalized_cat_dims).intersection(fidelity_dims):
            raise ValueError("cat_dims and fidelity dimensions must be disjoint.")
