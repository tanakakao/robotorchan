...
class HistGradientBoostingSurrogate(NonGPModelMixin, Model, nn.Module):
    """Histogram gradient boosting with bootstrap model uncertainty.

    Posterior members are complete independently fitted boosting models, not
    internal boosting iterations or individual trees.
    """
    ...
    def __init__(
        ...
        **boosting_kwargs: Any,
        ...
        if train_Y.ndim != 2 or train_Y.shape[-1] != 1:
            raise ValueError("HistGradientBoostingSurrogate supports one output only.")
        ...
        self._members: list[HistGradientBoostingRegressor] = []
    ...
    def fit(self) -> None:
        """Fit independent histogram boosting models on bootstrap resamples."""
        ...
        for member_index in range(self.n_members):
            ...
            member = HistGradientBoostingRegressor(
                random_state=(
    ...
    def posterior(
        ...
        **kwargs: Any,
        ...
        if output_indices not in (None, [0]):
            raise ValueError("HistGradientBoostingSurrogate has only output index 0.")
        if observation_noise is not False:
            raise NotImplementedError(
                "HistGradientBoostingSurrogate does not model observation noise."
            )
