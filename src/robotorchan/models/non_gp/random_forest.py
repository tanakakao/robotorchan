"""Random-forest surrogate models for Bayesian optimization."""
...
class RandomForestSurrogate(NonGPModelMixin, Model, nn.Module):
    """BoTorch-compatible random-forest regression surrogate."""
    ...
    def __init__(
        ...
        **forest_kwargs: Any,
        ...
        if RandomForestRegressor is None:
            raise ImportError(
                "RandomForestSurrogate requires scikit-learn. Install the robotorchan tree extra."
            )
        if train_X.ndim != 2:
    ...
    def is_fitted(self) -> bool:
        """Whether the underlying forest has been fitted."""
        return self._is_fitted

    def fit(self) -> None:
        """Fit the random forest using constructor-level raw training data."""
    ...
    def posterior(
        ...
        **kwargs: Any,
        ...
        if output_indices not in (None, [0]):
            raise ValueError("RandomForestSurrogate has only output index 0.")
