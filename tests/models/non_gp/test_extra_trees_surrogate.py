...
from robotorchan.models import ExtraTreesSurrogate
...
def test_extra_trees_training_contract_and_raw_data() -> None:
    train_X, train_Y = make_training_data()
    model = ExtraTreesSurrogate(train_X, train_Y, n_estimators=8, random_state=0)
...
def test_extra_trees_posterior_uses_tree_ensemble() -> None:
    train_X, train_Y = make_training_data()
    model = ExtraTreesSurrogate(train_X, train_Y, n_estimators=7, random_state=1)
    model.fit()
...
def test_extra_trees_posterior_supports_batched_candidates() -> None:
    train_X, train_Y = make_training_data()
    model = ExtraTreesSurrogate(train_X, train_Y, n_estimators=5, random_state=2)
    model.fit()

    posterior = model.posterior(torch.rand(3, 4, 1, dtype=torch.double))

    assert posterior.values.shape == torch.Size([3, 5, 4, 1])


def test_extra_trees_integrates_with_mc_qei() -> None:
    train_X, train_Y = make_training_data()
    model = ExtraTreesSurrogate(train_X, train_Y, n_estimators=8, random_state=3)
    model.fit()
...
def test_extra_trees_requires_fit_and_single_output() -> None:
    train_X, train_Y = make_training_data()
    model = ExtraTreesSurrogate(train_X, train_Y, n_estimators=4, random_state=4)
    with pytest.raises(RuntimeError, match="fit"):
        model.posterior(train_X[:2])
    with pytest.raises(ValueError, match="one output"):
        ExtraTreesSurrogate(train_X, torch.cat([train_Y, train_Y], dim=-1))
