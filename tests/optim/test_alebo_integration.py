...
def test_alebo_model_and_strategy_complete_one_bo_step() -> None:
    bounds = torch.stack(
        [torch.zeros(6, dtype=torch.double), torch.ones(6, dtype=torch.double)]
    )
    strategy = ALEBOStrategy(
        bounds, embedding_dim=2, seed=7, num_restarts=3, raw_samples=32
    )
    train_Z = torch.tensor(
        [[0.0, 0.0], [0.25, -0.1], [-0.2, 0.2], [0.1, 0.3]], dtype=torch.double
    )
